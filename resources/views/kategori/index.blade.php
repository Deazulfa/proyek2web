...
@section('content')
    <div class="container mt-5">
        <h1 style="margin-bottom: 50px">Kategori Produk</h1>
                ...
                @foreach ($category as $item)
                    <tr>
                        ...
                        <td class="d-flex justify-content-beetwen">
                            <a href="{{ route('category.edit', $item->id) }}" class="btn btn-warning me-2">
                                Edit
                            </a>
                            ...
                            <form action="{{ route('category.destroy', $item->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger text-light" onclick="return confirm('Are you sure you want to delete this category?')">Delete</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
